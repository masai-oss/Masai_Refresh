...
const StartQuizModal = ({ modalData, handleClose }) => {
  ...
  const startQuiz = async () => {
    let res = await dispatch(
      questionActions.attemptQuiz({ topic_id: topicId, topic })
    );
    if (res.output) {
      let state = res.state;
      ...
      history.replace(
        `/quiz_questions?attempt_id=${attempt_id}&submission_id=${submission_id}&question_id=${question_id}&topic=${topic}`
      );
    } else {
  ...
  return (
    <div>
      <CustomDialog
        ...
        message={`You are about to start a quiz on ${topic
          .split("_")
          .join(" ")}. Are you sure you wanna go ahead`}
        okBtnTitle="Start"
